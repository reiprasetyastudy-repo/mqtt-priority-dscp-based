#!/bin/bash
###############################################################################
# Run Full Experiment - Scenario 08 DSCP QoS with Packet Loss (13 Switches)
#
# Starts controller + topology in one script with packet loss simulation
...
echo "=============================================================="
echo "  SCENARIO 08: DSCP QoS with Packet Loss (13 Switches)"
echo "=============================================================="
echo ""
echo "Topology: Hierarchical 3-Tier (13 switches, 19 hosts)"
echo "Packet Loss: 5% core, 2% edge (Wireless IoT simulation)"
echo "Bandwidth: 1.0 Mbps (108% utilization)"
...
RUN_RESULTS_DIR="$PROJECT_ROOT/results/08-dscp-qos-13switches-lossy/run_$TIMESTAMP"
mkdir -p "$RUN_RESULTS_DIR/logs"

echo "Results will be saved to: $RUN_RESULTS_DIR"
echo ""

cp "$PROJECT_ROOT/logs/ryu.log" "$RUN_RESULTS_DIR/logs/ryu_controller.log" 2>/dev/null || true

cd "$RUN_RESULTS_DIR"

{
    echo "=============================================================="
    echo "  SCENARIO 08: DSCP QoS - Lossy Network (13 Switches)"
    echo "=============================================================="
    echo ""
    echo "Topology: Hierarchical 3-Tier with Packet Loss (5% core, 2% edge)"
