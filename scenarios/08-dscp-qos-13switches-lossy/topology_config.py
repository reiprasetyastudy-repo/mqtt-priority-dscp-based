#!/usr/bin/env python3
"""
DSCP-Based Priority Topology - Scenario 08
Hierarchical 3-Tier with 13 Switches - Lossy Network (Wireless IoT)
...
Use Case: Wireless IoT Smart Building (3 floors, 3 rooms per floor)
- s1: Core/Gateway switch
- s2-s4: Aggregation switches (1 per floor)
- s5-s13: Edge/Access switches (1 per room, 9 total) - WIRELESS LINKS
- Each edge switch: 2 publishers (1 anomaly + 1 normal)
- Total: 18 publishers + 1 broker/subscriber

SCENARIO DIFFERENCE FROM 06:
- Scenario 06: Bandwidth congestion only (0.5 Mbps, no packet loss)
- Scenario 08: Moderate bandwidth (1.0 Mbps) + Packet Loss (5%)
  * Simulates REALISTIC WIRELESS IoT (WiFi, LoRa, Cellular)
  * Core links: 5% packet loss (wireless backbone)
  * Edge links: 2% packet loss (wireless sensor connectivity)
...
- DSCP priority TESTED UNDER ADVERSE CONDITIONS (loss + congestion)
...
Network Conditions: LOSSY (realistic wireless IoT)
"""
...
RESULTS_DIR = os.path.join(PROJECT_ROOT, 'results/08-dscp-qos-13switches-lossy')

# LOG_DIR will be set dynamically based on current working directory
LOG_DIR = None

# Ensure directories exist
os.makedirs(RESULTS_DIR, exist_ok=True)

# Network Configuration - LOSSY WIRELESS NETWORK
ENABLE_BANDWIDTH_LIMIT = True
ENABLE_PACKET_LOSS = True       # NEW: Enable packet loss simulation

# Bandwidth Configuration
LINK_BANDWIDTH_MBPS = 0.5       # 0.5 Mbps (SAME AS SCENARIO 06 for fair comparison)
                                # This isolates the effect of packet loss

# Packet Loss Configuration (Realistic Wireless IoT)
CORE_PACKET_LOSS_PCT = 20        # 5% loss at core links (wireless backbone/gateway)
EDGE_PACKET_LOSS_PCT = 10        # 2% loss at edge links (wireless sensors)
                                # WiFi: typical 1-5% loss
                                # LoRa: 5-15% loss outdoor
                                # Cellular IoT: 2-10% loss
...
MSG_RATE = 50                   # 50 msg/s per publisher
                                ...
                                # Plus: 5% random packet loss at core, 2% at edge
                                # Total challenge: EXTREME CONDITIONS (High Congestion + Packet Loss)
...
class DSCPHierarchicalTopology:
    ...
    def build(self):
        ...
        if ENABLE_BANDWIDTH_LIMIT and ENABLE_PACKET_LOSS:
            self.net.addLink(h_broker, s1,
                           cls=TCLink,
                           bw=LINK_BANDWIDTH_MBPS,
                           loss=CORE_PACKET_LOSS_PCT)
        elif ENABLE_BANDWIDTH_LIMIT:
            self.net.addLink(h_broker, s1, cls=TCLink, bw=LINK_BANDWIDTH_MBPS)
        else:
            self.net.addLink(h_broker, s1)
        ...
        # Connect aggregation to core (CORE LINKS - HIGH LOSS)
        if ENABLE_BANDWIDTH_LIMIT and ENABLE_PACKET_LOSS:
            self.net.addLink(s2, s1, cls=TCLink, bw=LINK_BANDWIDTH_MBPS, loss=CORE_PACKET_LOSS_PCT)
            self.net.addLink(s3, s1, cls=TCLink, bw=LINK_BANDWIDTH_MBPS, loss=CORE_PACKET_LOSS_PCT)
            self.net.addLink(s4, s1, cls=TCLink, bw=LINK_BANDWIDTH_MBPS, loss=CORE_PACKET_LOSS_PCT)
        elif ENABLE_BANDWIDTH_LIMIT:
            self.net.addLink(s2, s1, cls=TCLink, bw=LINK_BANDWIDTH_MBPS)
            self.net.addLink(s3, s1, cls=TCLink, bw=LINK_BANDWIDTH_MBPS)
            self.net.addLink(s4, s1, cls=TCLink, bw=LINK_BANDWIDTH_MBPS)
        else:
        ...
        for i in range(5, 8):  # s5, s6, s7
            ...
            if ENABLE_BANDWIDTH_LIMIT and ENABLE_PACKET_LOSS:
                self.net.addLink(sw, s2,
                               cls=TCLink,
                               bw=LINK_BANDWIDTH_MBPS,
                               loss=EDGE_PACKET_LOSS_PCT)
            elif ENABLE_BANDWIDTH_LIMIT:
                self.net.addLink(sw, s2, cls=TCLink, bw=LINK_BANDWIDTH_MBPS)
            else:
                self.net.addLink(sw, s2)
        ...
        for i in range(8, 11):  # s8, s9, s10
            ...
            if ENABLE_BANDWIDTH_LIMIT and ENABLE_PACKET_LOSS:
                ...
                self.net.addLink(sw, s3, cls=TCLink, bw=LINK_BANDWIDTH_MBPS)
            else:
                self.net.addLink(sw, s3)
        ...
        for i in range(11, 14):  # s11, s12, s13
            ...
            if ENABLE_BANDWIDTH_LIMIT and ENABLE_PACKET_LOSS:
                ...
                self.net.addLink(sw, s4, cls=TCLink, bw=LINK_BANDWIDTH_MBPS)
            else:
                self.net.addLink(sw, s4)
        ...
        for idx, sw in enumerate(edge_switches_f1):
            ...
            if ENABLE_BANDWIDTH_LIMIT:
                self.net.addLink(h_anomaly, sw, cls=TCLink, bw=LINK_BANDWIDTH_MBPS)
            else:
            ...
            if ENABLE_BANDWIDTH_LIMIT:
                self.net.addLink(h_normal, sw, cls=TCLink, bw=LINK_BANDWIDTH_MBPS)
            else:
    ...
    def start_mqtt_components(self, duration=None):
        ...
        env_vars = (
            ...
            f'SCENARIO_NAME="08-dscp-qos-13switches-lossy" '
            f'TOPOLOGY_TYPE="DSCP Hierarchical 3-Tier - Lossy Network (13 switches, 19 hosts)" '
            f'NUM_SWITCHES=13 '
