#!/usr/bin/env python3
"""
DSCP-Based Priority Topology - Scenario 07
Hierarchical 3-Tier with 13 Switches - Core Bottleneck
...
RESULTS_DIR = os.path.join(PROJECT_ROOT, 'results/07-dscp-qos-13switches-core-bottleneck')
...
CORE_BW_MBPS = 0.5              # Core switch (s1) bottleneck: 0.5 Mbps
OTHER_BW_MBPS = 10              # Other links (edge-agg, agg-core): 10 Mbps
MSG_RATE = 50                   # 50 msg/s per publisher
                                # Calculation: 18 publishers × 50 msg/s × 150 bytes × 8 bits
                                #            = 1,080,000 bps = 1.08 Mbps
                                # Core Utilization: 1080 Kbps / 500 Kbps = 216% (VERY HIGH CONGESTION!)
                                # Other Links: 1080 Kbps / 10,000 Kbps = 10.8% (NO CONGESTION)
...
class DSCPHierarchicalTopology:
    ...
    def build(self):
        ...
        if ENABLE_BANDWIDTH_LIMIT:
            ...
            info(f'*** Bandwidth limiting ENABLED:\n')
            info(f'    - Core (s1) links: {CORE_BW_MBPS} Mbps (BOTTLENECK)\n')
            info(f'    - Other links: {OTHER_BW_MBPS} Mbps\n')
        else:
        ...
        if ENABLE_BANDWIDTH_LIMIT:
            self.net.addLink(h_broker, s1, bw=CORE_BW_MBPS)  # Core bottleneck
        else:
        ...
        # Connect aggregation to core (BOTTLENECK - connects to s1)
        if ENABLE_BANDWIDTH_LIMIT:
            self.net.addLink(s2, s1, bw=CORE_BW_MBPS)  # Core bottleneck
            self.net.addLink(s3, s1, bw=CORE_BW_MBPS)  # Core bottleneck
            self.net.addLink(s4, s1, bw=CORE_BW_MBPS)  # Core bottleneck
        else:
        ...
        for i in range(5, 8):  # s5, s6, s7
            ...
            if ENABLE_BANDWIDTH_LIMIT:
                self.net.addLink(sw, s2, bw=OTHER_BW_MBPS)  # High capacity
            else:
        ...
        for i in range(8, 11):  # s8, s9, s10
            sw = self.net.addSwitch(f's{i}', protocols='OpenFlow13')
            if ENABLE_BANDWIDTH_LIMIT:
                self.net.addLink(sw, s3, bw=OTHER_BW_MBPS)  # High capacity
            else:
        ...
        for i in range(11, 14):  # s11, s12, s13
            sw = self.net.addSwitch(f's{i}', protocols='OpenFlow13')
            if ENABLE_BANDWIDTH_LIMIT:
                self.net.addLink(sw, s4, bw=OTHER_BW_MBPS)  # High capacity
            else:
        ...
        for idx, sw in enumerate(edge_switches_f1):
            ...
            if ENABLE_BANDWIDTH_LIMIT:
                self.net.addLink(h_anomaly, sw, bw=OTHER_BW_MBPS)  # High capacity
            else:
            ...
            if ENABLE_BANDWIDTH_LIMIT:
                self.net.addLink(h_normal, sw, bw=OTHER_BW_MBPS)  # High capacity
            else:
        ...
        for switch_name in switches:
            try:
                ...
                for port in ports:
                    ...
                    # Core switch (s1) uses core bandwidth, others use higher bandwidth
                    if switch_name == 's1':
                        max_rate = int(CORE_BW_MBPS * 1000000)  # Core bottleneck
                        bw_label = f"{CORE_BW_MBPS} Mbps (CORE)"
                    else:
                        max_rate = int(OTHER_BW_MBPS * 1000000)  # High capacity
                        bw_label = f"{OTHER_BW_MBPS} Mbps"
                    ...
                    if result.returncode == 0:
                        info(f'  ✓ {switch_name}:{port} [{bw_label}]\n')
    ...
    def start_mqtt_components(self, duration=None):
        ...
        env_vars = (
            ...
            f'LINK_BANDWIDTH_MBPS={CORE_BW_MBPS} '
            f'ENABLE_QOS_QUEUES=True '
            f'SCENARIO_NAME="07-dscp-qos-13switches-core-bottleneck" '
            f'TOPOLOGY_TYPE="DSCP Hierarchical 3-Tier - Core Bottleneck (13 switches, 19 hosts)" '
            f'NUM_SWITCHES=13 '
        ...
        for idx, pub_info in enumerate(self.publishers):
            ...
            # IMPORTANT: Use DSCP publishers from scenario 07 directory!
            if pub_type == 'anomaly':
...
def main():
    """Main function to run the DSCP hierarchical topology with core bottleneck"""
    parser = argparse.ArgumentParser(description='DSCP-Based Hierarchical Topology - Scenario 07 (Core Bottleneck)')
