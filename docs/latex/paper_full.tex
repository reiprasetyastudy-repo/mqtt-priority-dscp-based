...
\hline
Messages Received & 53,673 & 11,726 \\
\hline
Avg Delay (ms) & 213.92 & 237,611.75 \\
\hline
Min Delay (ms) & 0.71 & 0.94 \\
\hline
Max Delay (ms) & 319.85 & 475,515.26 \\
\hline
Std Dev Delay (ms) & 43.03 & 136,356.80 \\
\hline
Avg Jitter (ms) & 60.76 & 530.80 \\
\hline
...
Hasil menunjukkan bahwa traffic anomaly dengan DSCP 46 mendapatkan delay rata-rata 213.92 ms dengan packet loss 0\%, sedangkan traffic normal dengan DSCP 0 mengalami delay rata-rata 237.6 detik (lebih dari 1000x lebih lambat) dan packet loss 78.18\%. Perbedaan signifikan ini membuktikan bahwa mekanisme DSCP-based QoS berhasil memprioritaskan traffic anomaly pada kondisi congestion.

\subsection{Skenario 02: Lossy Network (13 Switches)}

Skenario ini menguji efektivitas mekanisme prioritas pada kondisi lossy network dengan packet loss tambahan pada link. Tabel \ref{tab:lossy} menunjukkan hasilnya.

\begin{table}[htbp]
\caption{Hasil Skenario Lossy Network (Rata-rata 3 Run)}
\begin{center}
\begin{tabular}{|l|r|r|}
\hline
\textbf{Metric} & \textbf{Anomaly} & \textbf{Normal} \\
 & \textbf{(DSCP 46)} & \textbf{(DSCP 0)} \\
\hline
Messages Received & 53,629 & 11,617 \\
\hline
Avg Delay (ms) & 338.21 & 237,683.68 \\
\hline
Min Delay (ms) & 0.51 & 0.62 \\
\hline
Max Delay (ms) & 7,541.84 & 490,781.51 \\
\hline
Std Dev Delay (ms) & 289.96 & 136,477.28 \\
\hline
Avg Jitter (ms) & 103.67 & 534.66 \\
\hline
Packet Loss (\%) & 0.01 & 78.37 \\
\hline
\end{tabular}
\label{tab:lossy}
\end{center}
...
Pada kondisi lossy network, traffic anomaly tetap mendapatkan prioritas dengan packet loss hanya 0.01\%. Delay rata-rata meningkat menjadi 338.21 ms (dibanding 213.92 ms pada baseline) karena adanya retransmisi akibat packet loss pada link. Namun, perbedaan dengan traffic normal tetap sangat signifikan.

\subsection{Skenario 05: Dual-Redundant (17 Switches)}

Skenario ini menguji mekanisme prioritas pada topologi dengan redundansi penuh di semua layer menggunakan 17 switches dengan STP enabled. Tabel \ref{tab:redundant} menunjukkan hasilnya.

\begin{table}[htbp]
\caption{Hasil Skenario Dual-Redundant (Rata-rata 3 Run)}
\begin{center}
\begin{tabular}{|l|r|r|}
\hline
\textbf{Metric} & \textbf{Anomaly} & \textbf{Normal} \\
 & \textbf{(DSCP 46)} & \textbf{(DSCP 0)} \\
\hline
Messages Received & 53,676 & 11,686 \\
\hline
Avg Delay (ms) & 212.76 & 237,891.29 \\
\hline
Min Delay (ms) & 0.74 & 0.69 \\
\hline
Max Delay (ms) & 362.73 & 475,505.73 \\
\hline
Std Dev Delay (ms) & 42.97 & 136,548.69 \\
\hline
Avg Jitter (ms) & 60.85 & 532.68 \\
\hline
Packet Loss (\%) & 0.00 & 78.25 \\
\hline
\end{tabular}
\label{tab:redundant}
\end{center}
...
Hasil menunjukkan performa yang sangat mirip dengan skenario baseline. Traffic anomaly tetap mendapatkan delay rendah (212.76 ms) dan packet loss 0\%. Hal ini membuktikan bahwa mekanisme prioritas bekerja dengan baik pada topologi yang lebih kompleks dengan redundansi.

\subsection{Skenario 06: Distribution Failure (17 Switches)}

Skenario ini menguji ketahanan mekanisme prioritas ketika terjadi kegagalan pada distribution layer. Switch distribusi dimatikan pada menit ke-4 (40\% durasi eksperimen) untuk mensimulasikan network failure. Tabel \ref{tab:failure} menunjukkan hasilnya.

\begin{table}[htbp]
\caption{Hasil Skenario Distribution Failure (Rata-rata 3 Run)}
\begin{center}
\begin{tabular}{|l|r|r|}
\hline
\textbf{Metric} & \textbf{Anomaly} & \textbf{Normal} \\
 & \textbf{(DSCP 46)} & \textbf{(DSCP 0)} \\
\hline
Messages Received & 31,294 & 11,157 \\
\hline
Avg Delay (ms) & 28,848.30 & 233,839.60 \\
\hline
Min Delay (ms) & 0.39 & 0.84 \\
\hline
Max Delay (ms) & 317,984.65 & 515,749.42 \\
\hline
Std Dev Delay (ms) & 66,728.73 & 136,797.09 \\
\hline
Avg Jitter (ms) & 169.99 & 569.65 \\
\hline
Packet Loss (\%) & 41.73 & 79.23 \\
\hline
\end{tabular}
\label{tab:failure}
\end{center}
...
Pada kondisi failure, kedua jenis traffic mengalami degradasi performa. Traffic anomaly mengalami packet loss 41.73\% karena message yang sedang dalam perjalanan saat failure terjadi. Namun, traffic anomaly tetap mendapatkan perlakuan lebih baik dibanding traffic normal (79.23\% loss). Delay rata-rata anomaly meningkat signifikan (28.8 detik) karena waktu yang dibutuhkan untuk STP reconvergence.

\subsection{Perbandingan Antar Skenario}

Tabel \ref{tab:comparison} menyajikan perbandingan komprehensif hasil semua skenario.

\begin{table}[htbp]
\caption{Perbandingan Hasil Semua Skenario}
\begin{center}
...
Dari tabel perbandingan terlihat bahwa:
\begin{itemize}
\item Pada kondisi normal (skenario 01, 02, 05), traffic anomaly selalu mendapatkan delay $<$350 ms dan packet loss $\approx$0\%.
\item Traffic normal konsisten mengalami delay $\approx$237 detik dan packet loss $\approx$78\% pada semua skenario tanpa failure.
\item Skenario failure menunjukkan degradasi pada kedua traffic, namun mekanisme prioritas tetap memberikan perlakuan lebih baik untuk anomaly.
\end{itemize}

\subsection{Analisis Per-Sensor}

Untuk memahami distribusi performa secara lebih detail, Tabel \ref{tab:persensor} menyajikan hasil per-sensor pada skenario baseline. Data menunjukkan konsistensi mekanisme prioritas pada setiap sensor individual.
...
\hline
\textbf{Sensor} & \textbf{Floor} & \textbf{Sent} & \textbf{Recv} & \textbf{Avg Delay} \\
\hline
\multicolumn{5}{|c|}{\textbf{Anomaly Traffic (DSCP 46)}} \\
\hline
sensor\_f1r1\_anomaly & 1 & 5,968 & 5,968 & 212.96 ms \\
sensor\_f1r2\_anomaly & 1 & 5,967 & 5,967 & 218.38 ms \\
sensor\_f1r3\_anomaly & 1 & 5,966 & 5,966 & 214.08 ms \\
sensor\_f2r1\_anomaly & 2 & 5,965 & 5,965 & 219.73 ms \\
sensor\_f2r2\_anomaly & 2 & 5,964 & 5,964 & 209.80 ms \\
sensor\_f2r3\_anomaly & 2 & 5,963 & 5,963 & 210.74 ms \\
sensor\_f3r1\_anomaly & 3 & 5,962 & 5,962 & 213.47 ms \\
sensor\_f3r2\_anomaly & 3 & 5,961 & 5,961 & 214.45 ms \\
sensor\_f3r3\_anomaly & 3 & 5,960 & 5,960 & 206.75 ms \\
\hline
\multicolumn{5}{|c|}{\textbf{Normal Traffic (DSCP 0)}} \\
\hline
sensor\_f1r1\_normal & 1 & 5,975 & 1,306 & 237,218 ms \\
sensor\_f1r2\_normal & 1 & 5,973 & 1,315 & 236,020 ms \\
sensor\_f1r3\_normal & 1 & 5,973 & 1,301 & 237,364 ms \\
sensor\_f2r1\_normal & 2 & 5,972 & 1,304 & 237,714 ms \\
sensor\_f2r2\_normal & 2 & 5,971 & 1,315 & 238,677 ms \\
sensor\_f2r3\_normal & 2 & 5,970 & 1,313 & 238,274 ms \\
sensor\_f3r1\_normal & 3 & 5,968 & 1,305 & 237,470 ms \\
sensor\_f3r2\_normal & 3 & 5,967 & 1,296 & 238,434 ms \\
...
Dari data per-sensor dapat dilihat bahwa:
\begin{itemize}
...
\item Sensor normal mengalami packet loss konsisten sekitar 78\% di semua floor dan room.
\item Tidak ada bias terhadap lokasi sensor tertentu dalam topologi.
\end{itemize}

\subsection{Analisis Per-Floor}

Tabel \ref{tab:perfloor} menyajikan agregasi hasil per-lantai untuk melihat apakah ada perbedaan performa berdasarkan posisi dalam topologi hierarchical.
...
Hasil menunjukkan bahwa tidak ada perbedaan signifikan antar lantai. Floor 3 yang paling jauh dari core switch memiliki delay anomaly sedikit lebih rendah (211.56 ms) dibanding Floor 1 (215.14 ms), yang menunjukkan bahwa jarak hop tidak mempengaruhi prioritas secara signifikan.

\subsection{Analisis Throughput}

Tabel \ref{tab:throughput} menyajikan perbandingan throughput keseluruhan pada setiap skenario.

\begin{table}[htbp]
\caption{Perbandingan Throughput Antar Skenario}
\begin{center}
\begin{tabular}{|l|r|r|r|}
\hline
\textbf{Skenario} & \textbf{Total Msg} & \textbf{Duration (s)} & \textbf{Throughput} \\
\hline
01-Baseline & 65,398 & 132.15 & 494.87 msg/s \\
\hline
02-Lossy & 65,246 & 128.00 & 509.97 msg/s \\
\hline
05-Redundant & 65,362 & 131.20 & 498.20 msg/s \\
\hline
06-Failure & 42,452 & 108.51 & 391.21 msg/s \\
\hline
\end{tabular}
...
Throughput pada skenario normal (01, 02, 05) konsisten sekitar 495-510 msg/s, mendekati kapasitas teoritis sistem (18 publisher × 10 msg/s = 180 msg/s input, dengan faktor queue dan processing). Pada skenario failure, throughput turun menjadi 391.21 msg/s karena message yang hilang selama periode failure dan reconvergence.

\subsection{Analisis Jitter}

Tabel \ref{tab:jitter} menyajikan perbandingan jitter antar skenario. Jitter merepresentasikan variasi delay yang penting untuk aplikasi real-time.

\begin{table}[htbp]
\caption{Perbandingan Jitter Antar Skenario}
\begin{center}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Skenario} & \textbf{Anomaly Jitter} & \textbf{Normal Jitter} & \textbf{Ratio} \\
\hline
01-Baseline & 60.76 ms & 530.80 ms & 8.7x \\
\hline
02-Lossy & 103.67 ms & 534.66 ms & 5.2x \\
\hline
05-Redundant & 60.85 ms & 532.68 ms & 8.8x \\
\hline
06-Failure & 169.99 ms & 569.65 ms & 3.4x \\
\hline
\end{tabular}
\label{tab:jitter}
\end{center}
...
Traffic anomaly memiliki jitter yang jauh lebih rendah dan stabil (60-170 ms) dibandingkan traffic normal (530-570 ms). Pada kondisi normal, rasio jitter mencapai 8.7-8.8x, menunjukkan bahwa traffic prioritas tidak hanya mendapatkan delay lebih rendah tetapi juga lebih predictable.

\subsection{QoS Improvement Metrics}

Tabel \ref{tab:qosimprovement} merangkum improvement yang dicapai oleh mekanisme DSCP-based QoS.
...
\hline
Message Delivered & 53,673 & 11,726 & 4.6x more \\
\hline
\end{tabular}
\label{tab:qosimprovement}
\end{center}
\end{table}

Hasil menunjukkan improvement yang sangat signifikan:
\begin{itemize}
\item \textbf{Delay}: Traffic anomaly 1,110x lebih cepat dari normal.
\item \textbf{Packet Loss}: 100\% message anomaly terkirim vs hanya 21.82\% message normal.
\item \textbf{Jitter}: Variasi delay 8.7x lebih rendah untuk traffic prioritas.
\item \textbf{Reliability}: 4.6x lebih banyak message anomaly yang berhasil diterima.
\end{itemize}

\section{Discussion}

Hasil eksperimen dari empat skenario menunjukkan bahwa kerangka kerja DSCP-based QoS berhasil mencapai tujuan utama penelitian. Berikut analisis mendalam dari setiap aspek:

\subsection{Efektivitas Mekanisme Prioritas}

Pada kondisi operasi normal (skenario 01, 02, 05), traffic anomaly dengan DSCP 46 konsisten mendapatkan delay rata-rata 212-338 ms dan packet loss mendekati 0\%. Sementara itu, traffic normal dengan DSCP 0 mengalami delay rata-rata 237 detik (lebih dari 1000x lebih lambat) dan packet loss sekitar 78\%. Perbedaan yang sangat signifikan ini membuktikan bahwa mekanisme HTB queue dengan alokasi bandwidth 60-80\% untuk Queue 1 efektif dalam melindungi traffic prioritas tinggi.

\subsection{Ketahanan Terhadap Kondisi Lossy}

Skenario 02 menunjukkan bahwa pada kondisi lossy network, mekanisme prioritas tetap efektif. Meskipun delay rata-rata anomaly meningkat dari 213.92 ms menjadi 338.21 ms (peningkatan 58\%), packet loss tetap hanya 0.01\%. Peningkatan delay ini disebabkan oleh retransmisi pada layer TCP, namun tidak mempengaruhi reliabilitas pengiriman message prioritas tinggi.

\subsection{Skalabilitas pada Topologi Kompleks}

Skenario 05 dengan topologi dual-redundant 17 switches menunjukkan performa yang hampir identik dengan skenario baseline (delay 212.76 ms vs 213.92 ms). Hal ini membuktikan bahwa mekanisme DSCP-based QoS dapat di-scale ke topologi yang lebih kompleks tanpa degradasi performa.

\subsection{Ketahanan Terhadap Failure}

Skenario 06 menunjukkan bahwa pada kondisi failure, kedua jenis traffic mengalami degradasi. Namun, traffic anomaly tetap mendapatkan perlakuan lebih baik dengan packet loss 41.73\% dibanding traffic normal (79.23\%). Tingginya packet loss anomaly pada skenario ini disebabkan oleh message yang sedang dalam perjalanan saat failure terjadi dan waktu yang dibutuhkan untuk STP reconvergence (30-45 detik).

\subsection{Perbandingan dengan Pendekatan Lain}

Dibandingkan dengan pendekatan broker-level seperti p-MQTT \cite{b3} yang hanya memprioritaskan di application layer, pendekatan ini memberikan proteksi QoS di network layer sehingga traffic prioritas terlindungi sejak meninggalkan publisher. Dibandingkan dengan RT-MQTT \cite{b2} yang memerlukan middleware tambahan dan deep packet inspection, pendekatan DSCP marking lebih sederhana dan scalable karena memanfaatkan mekanisme DiffServ standar yang sudah didukung oleh perangkat jaringan.

\section{Conclusion}

Penelitian ini mengusulkan dan memvalidasi kerangka kerja QoS berbasis DSCP untuk prioritas traffic MQTT pada jaringan Software-Defined Networking. Hasil eksperimen pada empat skenario yang berbeda menunjukkan efektivitas pendekatan ini:

\begin{itemize}
\item Pada kondisi operasi normal (skenario baseline, lossy, dan dual-redundant), traffic anomaly konsisten mendapatkan delay 212-338 ms dan packet loss mendekati 0\%, dibandingkan traffic normal dengan delay 237 detik dan packet loss 78\%.
\item Mekanisme prioritas tetap efektif pada topologi kompleks dengan 17 switches dan STP enabled.
\item Pada kondisi failure, traffic anomaly tetap mendapatkan perlakuan lebih baik (41.73\% loss vs 79.23\% untuk normal).
\end{itemize}

Kontribusi utama penelitian ini meliputi: (1) implementasi DSCP marking pada publisher MQTT menggunakan socket option yang transparan terhadap protokol MQTT, (2) integrasi dengan SDN controller untuk automatic queue assignment berdasarkan DSCP value, (3) validasi eksperimental pada empat skenario berbeda dengan kondisi congestion realistis, dan (4) demonstrasi skalabilitas pada topologi dengan redundansi penuh.

Future work dapat mencakup: (a) evaluasi pada topologi dengan jumlah switch lebih besar, (b) implementasi dynamic priority adjustment berdasarkan kondisi jaringan, dan (c) integrasi dengan mekanisme Time-Sensitive Networking (TSN) untuk aplikasi industrial yang memerlukan deterministic latency.
